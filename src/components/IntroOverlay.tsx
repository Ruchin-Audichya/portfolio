...
export function IntroOverlay({ onComplete }: IntroOverlayProps) {
  ...
  useEffect(() => {
    const greetingTimer = window.setTimeout(() => setStep("explore"), 600);
  ...
  useEffect(() => {
    ...
    // Auto-continue so first-time visitors and automated checks don't get stuck on a dark gate.
    const autoContinueTimer = window.setTimeout(() => onComplete(), 2600);
    return () => window.clearTimeout(autoContinueTimer);
  }, [onComplete, step]);

  const handleExplore = () => onComplete();
        ...
        <AnimatePresence mode="wait">
          {step === "greeting" && (
            ...
            >
              ...
              <h1 className="text-4xl md:text-6xl font-black tracking-tighter text-white">Hello...</h1>
            </motion.div>
          )}
